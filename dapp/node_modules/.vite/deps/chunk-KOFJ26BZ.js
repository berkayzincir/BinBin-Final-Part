...
async function getSocket(url_) {
  ...
  const { schedule } = createBatchScheduler({
    ...
    fn: async () => {
      var _a;
      let WebSocket = await import("./browser-ODXOZQ23.js");
...
function format3(bytes, to) {
  if (Array.isArray(bytes))
    return bytes.map((b) => format3(b, to));
...
async function recoverPublicKey({ hash, signature }) {
  ...
  if (v === 0 || v === 1)
    v += 27;
  const { secp256k1 } = await import("./secp256k1-TG2BNPZH.js");
...
//# sourceMappingURL=chunk-KOFJ26BZ.js.map
